<?php
    ...
    if (isset($_POST['search']) && isset($_POST['searchColumns'])) {
        ...
        $query .=  "AND `sounds`.`user_id` = :user_id ";
  ...
  //      $db = new DB;  // instantiated above
      ...
      //  $total = $r->rowcount;
        ...
        $query = "SELECT `sounds`.`id`,`sounds`.`description`,`sounds`.`timestamp`,`mp3_files`.`wave_length`,`mp3_files`.`filename`, `mp3_files`.`wave_byterate`,`mp3_files`.`wave_framerate` ";
        $query .= "FROM `sounds`, `sounds_mp3_files`,";
        $query .= "`mp3_files`,`keywords`,`sounds_keywords` ";
        $query .= "WHERE `sounds`.`id` = `sounds_mp3_files`.`sound_id` ";
        $query .=  "AND `mp3_files`.`id` = `sounds_mp3_files`.`mp3_file_id`";
...
//        $result = DataManager::DBQuery($stmt);
      ...
      $query = "SELECT `sounds`.`id`,`sounds`.`timestamp`, `sounds`.`description`, `mp3_files`.`wave_length`,`mp3_files`.`filename`,`mp3_files`.`wave_byterate`,`mp3_files`.`wave_framerate`
    FROM `sounds` , `sounds_mp3_files` , `mp3_files`
    WHERE `sounds`.`id` = `sounds_mp3_files`.`sound_id`
    AND `mp3_files`.`id` = `sounds_mp3_files`.`mp3_file_id` 
    AND `sounds`.`user_id` = :user_id ORDER BY $sorton $sortby $limit";
$statement_number = "3";
    ...
    $query = "SELECT `sounds`.`id`,`sounds`.`timestamp`, `sounds`.`description`, `mp3_files`.`wave_length`,`mp3_files`.`filename`,`mp3_files`.`wave_byterate` 
    FROM `sounds` , `sounds_mp3_files` , `mp3_files`
    WHERE `sounds`.`id` = `sounds_mp3_files`.`sound_id`
    AND `mp3_files`.`id` = `sounds_mp3_files`.`mp3_file_id`
    AND `sounds`.`user_id` = :user_id $limit";
        $statement_number = "4";
        }
    ...
    foreach($ret as $row){
      ...
      if(@count($sound->mp3_files)){
            $row->has_mp3 = '<img onclick="mp3Click(datagrid)" class="mp3_check" src="./images/12-em-check.png" />';
      } else {
            $row->has_mp3 = '';
      }
